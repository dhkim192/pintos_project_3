...
start_process(void *pcb_)
{
    ...
    virtual_memory_init(&thread_current()->virtual_memory);
             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        memset(entry, 0, sizeof(struct virtual_memory_entry));
        entry->vaddr = upage;
        entry->virtual_memory = BINARY_FILE;
...
setup_stack(void **esp)
{
    ...
    struct frame * temp_frame = frame_alloc(((uint8_t *) PHYS_BASE) - PGSIZE);

    if (!install_page(temp_frame->upage,temp_frame->kpage,true)) {
    ...
    memset (entry, 0, sizeof(struct virtual_memory_entry));
    entry->vaddr = temp_frame->upage;
    entry->kpage = temp_frame->kpage;
