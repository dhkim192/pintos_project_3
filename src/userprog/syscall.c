...
syscall_handler(struct intr_frame *f)
{
    ...
    case SYS_MMAP:
    {
        ...
        check_vaddr(esp + sizeof(uintptr_t));
        check_vaddr(esp + 3 * sizeof(uintptr_t) - 1);
        handle = *(int *)(esp + sizeof(uintptr_t));
        addr = *(void **)(esp + 2 * sizeof(uintptr_t));

        f->eax = syscall_mmap(handle,addr);
        break;
    }
    case SYS_MUNMAP:
    {
        ...
        check_vaddr(esp + sizeof(uintptr_t));
        check_vaddr(esp + 2 * sizeof(uintptr_t) - 1);
        mapping = *(int *)(esp + sizeof(uintptr_t));
...
check_vaddr(const void *vaddr)
{
    ...
    if (!pagedir_get_page(thread_get_pagedir(), vaddr)) {
        if (!virtual_memory_entry_find(pg_round_down(vaddr))) {
            syscall_exit(-1);
        }
    }
}
